...
//
	...
	strcat(bufd,"\",\"model\":\"ZMAi-90\",\"sw_version\":\"");
	strcat(bufd,AP_VER);
	if (wbuff[0]) {
	strcat(bufd,"\",\"configuration_url\":\"http://");
	strcat(bufd,wbuff);
	}
	strcat(bufd,"\",\"manufacturer\":\"Tuya Smart Home Automation\"},");
...
//
	...
	esp_mqtt_client_publish(mqttclient, buft, bufd, 0, 1, 1);
//
	strcpy(buft,"homeassistant/button/");
	strcat(buft,MQTT_BASE_TOPIC);
	strcat(buft,"/1x");
	strcat(buft,tESP8266Addr);
	strcat(buft,"/config");
	...
	strcat(bufd,"\"command_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/status\",\"payload_press\":\"restart\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/status\"}");
	esp_mqtt_client_publish(mqttclient, buft, bufd, 0, 1, 1);
//
	...
	strcat(bufd,"\",\"manufacturer\":\"Tuya Smart Home Automation\"},");
	strcat(bufd,"\"device_class\":\"voltage\",\"state_class\":\"measurement\",\"state_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
...
//
	...
	strcat(bufd,"\",\"manufacturer\":\"Tuya Smart Home Automation\"},");
	strcat(bufd,"\"device_class\":\"current\",\"state_class\":\"measurement\",\"state_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
...
//
	...
	strcat(bufd,"\",\"manufacturer\":\"Tuya Smart Home Automation\"},");
	strcat(bufd,"\"device_class\":\"power\",\"state_class\":\"measurement\",\"state_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
...
//
	...
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/ApparentPower\",\"unit_of_meas\":\"VA\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/status\"}");
	esp_mqtt_client_publish(mqttclient, buft, bufd, 0, 1, 1);
//
//
	...
	strcat(bufd,"\",\"manufacturer\":\"Tuya Smart Home Automation\"},");
	strcat(bufd,"\"device_class\":\"power_factor\",\"state_class\":\"measurement\",\"state_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/PowerFactor\",\"unit_of_meas\":\"%\",\"availability_topic\":\"");
	strcat(bufd,MQTT_BASE_TOPIC);
	strcat(bufd,"/status\"}");
	esp_mqtt_client_publish(mqttclient, buft, bufd, 0, 1, 1);
	strcpy(buft,"homeassistant/sensor/");
...
*/
	...
	} else if (!memcmp(event->topic+topoff, "state", event->topic_len-topoff) && (mcmsnd < 376)) {
...
*/
	...
	if (s_retry_num < WIFI_MAXIMUM_RETRY) {
		esp_wifi_connect();
		if (floop && !(wf_bits & 0x02)) s_retry_num |= 0x01;
		else s_retry_num++;
...
//
	...
	if ((wf_bits & 0x30) == 0) strcat(bsend,"selected ");
	strcat(bsend,"value=\"0\">802.11b/g/n</option><option ");
	if ((wf_bits & 0x30) == 16) strcat(bsend,"selected ");
/*
	strcat(bsend,"value=\"16\">802.11bg</option><option ");
	if ((wf_bits & 0x30) == 32) strcat(bsend,"selected ");
	strcat(bsend,"value=\"32\">802.11b</option></select> Mode&emsp;<input type=\"checkbox\" name=\"wfb1\" value=\"1\"");
*/
	strcat(bsend,"value=\"16\">802.11bg</option></select> Mode&emsp;<input type=\"checkbox\" name=\"wfb1\" value=\"1\"");
	...
	strcat(bsend,"> Restart if WIFI loss&emsp;<input type=\"checkbox\" name=\"wfb3\" value=\"3\"");
	...
	if (ftrufal) strcat(bsend,"checked");
        strcat(bsend,"> \"true/false\" Response(\"ON/OFF\" only if Hass Discovery)&emsp;<input type=\"checkbox\" name=\"chk4\" value=\"4\"");
	if (bOff30s) strcat(bsend,"checked");
        strcat(bsend,"> Mqtt \"OFF\" switch to \"ON\" after 30sec delay</br>");
...
*/
	...
	buf3[0] = 0;
	strcpy(buf2,"swfmd");
	parsuri(buf1,buf3,buf2,1024,3);
	wf_bits &= 0xcf;
	wf_bits = wf_bits | (atoi(buf3) & 0x30);
	buf3[0] = 0;
	strcpy(buf2,"wfb1");
	parsuri(buf1,buf3,buf2,1024,2);
	wf_bits &= 0xfe;
...
// write nvs
	...
	nvs_set_u8(my_handle,  "wfbt",  wf_bits);
	nvs_set_u16(my_handle, "chk4",  bOff30s);
...
void rxlnparse()
{
	...
	if (!mqtdel && (bCount != bPCount) && mqttConnected) {
	...
	itoa(tdata,tbuff,10);
	strcat(MQTT_DATA,tbuff);
	if (!mqtdel && (bCurrent != bPCurrent) && mqttConnected) {
	esp_mqtt_client_publish(mqttclient, MQTT_TOPIC, MQTT_DATA, 0, 1, 1);
        bPCurrent = bCurrent;
	}
	break;
	...
	itoa(tdata,tbuff,10);
	strcat(MQTT_DATA,tbuff);
	if (!mqtdel && (bPower != bPPower) && mqttConnected) {
	esp_mqtt_client_publish(mqttclient, MQTT_TOPIC, MQTT_DATA, 0, 1, 1);
        bPPower = bPower;
	}
	break;
	...
	itoa(tdata,tbuff,10);
	strcat(MQTT_DATA,tbuff);
	if (!mqtdel && (bVoltage != bPVoltage) && mqttConnected) {
	esp_mqtt_client_publish(mqttclient, MQTT_TOPIC, MQTT_DATA, 0, 1, 1);
        bPVoltage = bVoltage;
	}
	break;
	...
	if (!mqtdel && (bState != bPState) && mqttConnected) {
	esp_mqtt_client_publish(mqttclient, MQTT_TOPIC, MQTT_DATA, 0, 1, 1);
	mcmsnd = 375;
        bPState = bState;
	}
...
// read nvs
	...
	nvs_get_u16(my_handle,  "chk4",  &bOff30s);
	nvs_get_u8(my_handle, "wfbt",   &wf_bits);
	size_t nvsize = 32;
...
// get esp mac addr
	...
	itoa(zmainum,tzbuf,10);
	strcat (tESP8266Addr,tzbuf);
...
//	uart_driver_install(UART_NUM_0, UART_BUF_SIZE * 2, 0, 0, NULL);
	uart_driver_install(UART_NUM_0, UART_BUF_SIZE * 1, UART_BUF_SIZE * 1, 0, NULL);
...
//
	...
	if (!mqtdel && (tuptime - t_last > 625)) {
//Calculate apparent power & cos fi
	bAPower = bVoltage * bCurrent / 1000;		

	if ((bAPower != bPAPower) && mqttConnected) {
	uint32_t tdata;
	char tbuff[16];
	char buft[32];
	char bufd[16];
	strcpy(buft,MQTT_BASE_TOPIC);
	strcat(buft,"/ApparentPower");
	tdata = bAPower / 10;
	itoa(tdata,tbuff,10);
	strcpy(bufd,tbuff);
	strcat(bufd,".");
	...
	bPowerF = (bAPower && bPower) ? (bPower * 100 / bAPower) : 0;
	if (bPowerF > 100) bPowerF = 100;
	if ((bPowerF != bPPowerF) && mqttConnected) {
//	uint32_t tdata;
	char tbuff[16];
	char buft[32];
	char bufd[16];
	strcpy(buft,MQTT_BASE_TOPIC);
	strcat(buft,"/PowerFactor");
/*  
	tdata = bPowerF / 100;
	itoa(tdata,tbuff,10);
	strcpy(bufd,tbuff);
...
//	wifi_ap_record_t wifidata;
	...
	if (floop && MQTT_SERVER[0] && (wf_bits & 0x04) && !s_retry_num && !f_update) {
	if (mqttConnected) {
...
*/
	...
	if (!mqtdel && (bCountT != bPCountT) && mqttConnected) {
	uint32_t tdata;
	char tbuff[16];
	char buft[32];
	char bufd[16];
	...
	if (!mqtdel && (bCountY != bPCountY) && mqttConnected) {
	uint32_t tdata;
	char tbuff[16];
	char buft[32];
	char bufd[16];
	...
	if (mqttConnected) esp_mqtt_client_stop(mqttclient);
	vTaskDelay(1000 / portTICK_PERIOD_MS);
	esp_restart();
}
